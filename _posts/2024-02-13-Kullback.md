...
$$
\mathrm{d} X_t= Z_2(X_t)\mathrm{d}t + \sqrt{2}\sum_{i=1}^m V_i \circ \mathrm{d}B^i_t=Z_2(X_t)\mathrm{d}t + \sqrt{2}\sum_{i=1}^m V_i \circ \left(\mathrm{d}\tilde{B}^i_t - u(X_t)\mathrm{d}t \right)=\sqrt{2}\sum_{i=1}^m V_i \circ \mathrm{d}\tilde{B}^i_t+(Z_2-\sum_{i=1}^m \sqrt{2}u^iV_i) \mathrm{d} t=Z_1(X_t)\mathrm{d}t + \sqrt{2}\sum_{i=1}^m V_i \circ \mathrm{d}\tilde{B}^i_t,
$$
...
where $$(\tilde{B})$$ is a $$m$$-dimensional standard Brownian motion under $$\mathbb{Q

}$$ defined by $$\frac{ \mathrm{d}\mathbb{Q}}{\mathrm{d} \mathbb{P}}= \mathcal{E}_T$$ with 
