...
#### Extension of a self-adjoint operator in $$L^2$$
...
Thus, there is a constant $$c_0 \ge 1$$ such that for all $$f \in L^\infty$$ and $$t>0$$,

 $$\| P_t f\|_\infty \le c_0e^{-\lambda t} \|f \|_\infty.$$
...
$$\| Tf \|_\infty \le \|T\|_{1 \rightarrow 2} \times \|f\|_2$$ for all $$f \in L^\infty$$. Hence, $$\|T\|_{2 \rightarrow \infty} \le \| T\|_{1\rightarrow 2}.$$
...
$$ P_tf(x) = \int_\mathbb{R} \frac{1}{\sqrt{2\pi t}} f(y+x)e^{-\frac{y^2}{2t}}\mathrm{d}y \quad \forall f \in L^\infty,$$

whose its unique invariant measure (up to a constant factor) is the Lebesgue measure $$\mathrm{d}x$$. 

It is clear that for all $$t>0$$, 
$$\| P_t f\|_{\infty} \le \frac{1}{\sqrt{2\pi t}} \int_\mathbb{R} | f(y)| \mathrm{d} y =\frac{1}{\sqrt{2\pi t}}  \| f\|_1.$$

Hence, the semigroup of a Brownian motion is ultracontractive.
Besides, it is clear that $$\| P_t f\|_\infty \le \| f\|_\infty$$ and $$\| P_tf\|_1 \le \int_{x \in \mathbb{R}} \int_{y \in\mathbb{R}} \frac{1}{\sqrt{2\pi t}}|f(y+x)|e^{-\frac{y^2}{2t}}\mathrm{d}y \mathrm{d} x=\|f\|_1$$
...
Indeed, let us consider the function $$f(x)=e^{i\lambda x}$$ for some real constant $$\lambda$$. Then, 

$$P_tf(x)= \mathbb{E}( e^{i\lambda (B_t+x)})=e^{i\lambda x} e^{-\frac{t\lambda^2}{2}}=e^{-\frac{t\lambda^2}{2}} f(x).$$ 
...
+ $$P_tf(x) =\int_\mathbb{R} \hat{f}(\lambda)e^{-\frac{t\lambda^2}{2}}$$    
Hence, by Plancherel's theorem again, we have:
+ $$\|P_tf\|_2^2 = \int_{\mathbb{R}} \| \hat{f}(\lambda) \|^2e^{-t\lambda^2} \mathrm{d} \lambda$$.
...
Let $$(P_t)$$ be the semigroup of the Ornstein-Uhlenbeck process $$(X_t)$$ driven by a Brownian motion $$(B_t)$$:
...
The generator $$\mathcal{L}$$ of $$(P_t)$$ is then $$\mathcal{L}f(x)=-xf'(x)+f''(x)$$, and for any $$f \in L^\infty$$,

$$P_tf(x)= \int_\mathbb{R} \frac{1}{\sqrt{2\pi}}f(y\sqrt{1-e^{-2t}}+ e^{-t}x)e^{-\frac{y^2}{2}}\mathrm{d}y.$$

Its invariant measure is then $$\mu(\mathrm{d}x)=e^{-\frac{x^2}{2}}\mathrm{d}x$$.
Clearly, $$(P_t)$$ is not ultracontractive. Indeed, by considering the function $$f(x)=|x|$$, we observe that for any $$t>0$$, $$\|f\|_1=\int_\mathbb{R} \frac{1}{\sqrt{2\pi}}|x|e^{-\frac{x^2}{2}} \mathrm{d}x=\sqrt{\frac{2}{\pi}}$$, whereas,

$$\lim_{x \rightarrow \infty} \frac{P_tf(x)}{|x|} \ge \lim_{x \rightarrow \infty} \int_\mathbb{R} \frac{1}{\sqrt{2\pi}}\frac{ e^{-t}|x|-|y|\sqrt{1-e^{-2t}}}{|x|}e^{-\frac{y^2}{2}}\mathrm{d}y=e^{-t}$$

Thus, $$\| P_t f\|_\infty = \infty$$.

Nonetheless, $$(P_t)$$ satisfies the Poincaré's inequality in $$L^2$$ for $$\lambda=1$$, indeed, the Gaussian Poincaré's inequality shows that for any standard normal variable $$X$$ and differentiable function $$f$$:
...
The last equation is due to the fact that $$\mathcal{L}$$ is the generator of $$P_t$$.
Hence, for all $$t \ge 0$$ and $$f \in L^2_0$$,

$$\| P_t f\|_2 \le e^{-t} \|f\|_2.$$


**Example 3** (Brownian motion on the circle ($$\mathbb{T}^1$$))


We now consider the Brownian motion $$(X_t)$$ on a circle and show that this Brownian motion is ultracontractive and satisfies Poincaré's inequality.

A Brownian motion $$(X_t)$$ on a circle, when embedded in $$\mathbb{R}^2$$, is a $$\mathbb{R}^2$$-valued stochastic process that can be represented as:

$$X_t= (\cos(2\pi B_t+\theta), \sin(2\pi B_t+\theta))$$

where $$(B_t)$$ is a standard Brownian motion.

Because a circle is isometric to a flat torus $$\mathbb{T}^1$$, we may also write $$X_t$$ as $$X_t= [B_t]$$, where $$[x]$$ denotes the projection of a real number $$x$$ into the quotient space $$\mathbb{R}/\mathbb{Z}.$$ To study this Brownian motion, we will also identify $$\mathbb{T}^1$$ with $$[0,1]$$. We also identify a continuous function $$f$$ on $$\mathbb{T}^1$$ with a function $$\tilde{f}: [0,1] \rightarrow \mathbb{R}$$ that is extendable to a continuous function on $$\mathbb{R}$$, i.e., $$f(0)=f(1)$$. Thus, by default, all the smooth functions on $$[0,1]$$ we considered in the rest of this example are extendable to a smooth periodic function on $$\mathbb{R}$$, i.e., $$f^{(n)}(0)=f^{(n)}(1)$$ for all $$n$$.

Let $$(P_t)$$ be the semigroup associated with $$(X_t)$$. Then for all smooth $$f$$:

$$P_tf(x)= \int_\mathbb{R} \frac{1}{\sqrt{2\pi t}} f([y+x])e^{-\frac{y^2}{2t}} \mathrm{d} y,$$

whose generator $$\mathcal{L}$$ is: $$\mathcal{L}f= f''$$. The unique invariant measure of $$(P_t)$$ is the Lebesgue measure on $$[0,1]$$.

Clearly, $$(P_t)$$ is ultracontractive because $$P_tf = \int_{[0,1]} f(x) g_t(x)\mathrm{d}x$$ with 

$$g_t(x)=\frac{1}{\sqrt{2\pi t}}\sum_{z \in \mathbb{Z}} e^{-\frac{(x+z)^2}{2t}},$$

and $$\| g_t\|_\infty < \infty$$ for all $$t>0$$.

Furthermore, for any $$1$$-periodic smooth function $$f$$, we have:

$$\langle f , \mathcal{L}f \rangle=\int_{[0,1]} ff''=-\int_{[0,1]} (f')^2 $$

Using Fourier series, we know that whenever $$\int_{[0,1]} f =0$$,

$$\int_{[0,1]} (f')^2 \ge (2\pi)^2 \int_{[0,1]} f^2$$

Hence, for all $$f \in L^2$$,

$$\| P_tf \|_2 \le e^{-2\pi^2 t} \|f\|_2.$$

Thus, there exists a constant $$c_0 \ge 1$$ such that for all $$f$$ such that $$\int f=0$$ and $$p \in [1,\infty]$$,

$$\| P_t f\|_p \le c_0 e^{-2\pi^2 t} \| f\|_p.$$
...
These examples illustrate various properties of stochastic processes and their semigroups, including ultracontractivity, the existence of invariant measures, and the satisfaction of Poincaré inequalities. The techniques used in these examples, such as Fourier series for periodic functions and the analysis of semigroups, are fundamental in the study of stochastic processes and partial differential equations.